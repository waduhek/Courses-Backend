...
from .utils import createSessionForUser


class TestAPIView(APIView):
    def post(self, request: Request, format=None) -> Response:
        session: Session = Session.objects.get(
            pk='6pt23fxbtf0ezeks60047186n585xy57'
        )
        serialiser: SessionSerialiser = SessionSerialiser(session)
        print(session.expire_date)
        
        response = Response()
        response.set_cookie(
            'sessionID', session.session_key, expires=session.expire_date
        )
...
class LoginView(APIView):
    def post(self, request: Request, format=None) -> Response:
        ...
        if user is not None:
            ...
            # TODO: Add ability to update session expiry date to prevent
            # multiple logins.
    ...
    def post(self, request: Request, format=None) -> Response:
        ...
        if request.COOKIES.get('sessionID') is not None:
            ...
            responseData: dict = {
                ...
                'the session ID is either in the body or set as a cookie.'
            }
